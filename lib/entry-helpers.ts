...
/** Legacy comma-split tags — still used for per-card "looking for" display. */
...
const CANONICAL_MAP = new Map<string, string>([
  ...
  // founder variants (standalone "founder"/"founders" merge into co-founder)
  ["founders", "Co Founder"],
  // strategist variants
  ...
  // strategy variants
  ["strategic", "Strategy"],
  // partnerships variants
  ["partnerships", "Partnerships"],
  ["partner", "Partnerships"],
  ["partners", "Partnerships"],
  // abbreviation / synonym merges
  ["go to market", "GTM"],
  ["strategist", "Strategy"],
  ["founder", "Co Founder"],
 ...
 *
 * Strategy:
 *  1. Check for known multi-word role phrases first (e.g. "founding engineer").
 *  2. Split remaining text into segments on commas, "and", "or", "/", "&".
 *  3. From each segment, extract significant single words (skip stop words).
 *  4. Title-case and de-duplicate the results.
 */
export function extractRoleKeywords(value: any): string[] {
  ...
  const addKeyword = (word: string) => {
    ...
    // Check canonical alias map first (collapses plurals/variants)
  ...
  for (const { pattern, label } of PHRASE_PATTERNS) {
    ...
    // Remove matched phrases so they don't get double-counted as individual words
